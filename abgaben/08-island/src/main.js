...
let withLightningLoc, withTextureLoc, withBumpmapLoc, isWaterLoc;
...
let sandTexture, sandNormalTexture, waterTexture;
let diffuseMapLoc, normalMapLoc, waterLoc;
...
function init() {
  ...
  const island = new Cube(
    from = {x: -2.5, y: -1.0, z: -2.5},
    to = {x: 2.5, y: 0.01, z: 2.5},
    colors = { ka: white, kd: white, ks: white },
    withLightning = true,
    withTexture = true,
    withBumpmap = true,
  )
  ...
  for (i = 0; i <= 3.5; i += 0.5) {
    const trunk = new Trunk(
      ...
      colors = { ka: brown, kd: white, ks: white },
      height = 0.5,
      withLightning = true
    )
  ...
  for (i = 0 ; i <= 360 ; i+= 45) {
    objects.push(new Leaf(
      ...
      colors = { ka: green, kd: white, ks: white },
      withLightning = true
    ));
  }
  ...
  let sunSphere = new Cube(
    ...
    to = { x: 0.2, y: 0.2, z: 0.2 },
    colors = { ka: yellow, kd: yellow, ks: yellow },
    withLightning = true,
  );
  ...
  // eye = vec3.fromValues(5.0, 5.0, 5.0);
  ...
  waterLoc = gl.getUniformLocation(program, "waterMap");
